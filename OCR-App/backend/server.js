...
/**
 * Route to request all names of people working at wiso in JSON format
 */
...
/**
 * Route to request detailed information about a specific person working at wiso (personal data, chair, offered lectures by that chair)
 */
...
/**
 * cronjob that gets all names of people working at wiso once a day to improve performance
...
const getLehrstuhlData = async(link) => {
    ...
    const personarr = await page.evaluate(() => {
        ...
        ).map((x) => x.outerHTML.slice(0, 3) + "target='_blank'" + x.outerHTML.slice(3, 9)  + x.outerHTML.slice(9));
    });
